...
from data.crop_utils import (
    project_box_from_crop,
    project_box_from_crop_center_norm,
    project_box_to_crop,
    project_box_to_crop_center_norm,
)
...
def test_project_box_center_norm_round_trip_stays_consistent():
    reference_box = np.asarray([40.0, 30.0, 50.0, 80.0], dtype=np.float32)
    search_box = np.asarray([55.0, 45.0, 45.0, 70.0], dtype=np.float32)

    crop_box = project_box_to_crop_center_norm(
        search_box_xywh=search_box,
        reference_box_xywh=reference_box,
        out_size=255,
        context_amount=0.5,
    ...
    restored_box = project_box_from_crop_center_norm(
        crop_box_cxcywh=crop_box,
        reference_box_xywh=reference_box,
        out_size=255,
        context_amount=0.5,
        center_override=(80.0, 85.0),
    ...
    assert np.allclose(np.asarray(restored_box, dtype=np.float32), search_box, atol=1.0)
    assert np.all(crop_box >= 0.0)
    assert np.all(crop_box <= 1.0)
